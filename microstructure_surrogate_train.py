# -*- coding: utf-8 -*-
"""
This is the microstructure surrogate training script, with multi-GPU support and pytorch tensordict based memmaps to minimize system RAM usage.
...
Needs Microstructure data stored locally, use:
    rsync -av --exclude *.tar.xz /gpfs/user/Dataset .
from scratch directory /scratch/mydir
Recommend using scratch drive if available to avoid networking overhead.

There was some speedup from compiling the model, but this has been temporarily disabled and removed due to issues with DDP.
Compiling could still be useful, particularly for inference without DDP.
...
# Model will be saved as a .pt file using this name. Logs of the training and testing accuracy will also be saved using this.
...
# Tell the model if it should run tests using data from the largest ensembles, for studying influence of ensemble size on accuracy.
...
def get_data_filenames(Testing, ensemble_size, ensemble_treatment, ensemble_comparison, Training, scratch_path, metadata_filename):
    ...
    """
    # The default list of standard ensemble sizes; should correspond to folder names in the MS_data directory.
...
def PreloadData(files, path: str, ensemble_comparison = False, input_width = 25):
    ...
    try:
        ...
        print('No matching TensorDict found, creating from files.')
        data_dict = {}
        data_dict["names"] = []
...
class SurrogateDataset(Dataset):
    ...
    def __init__(self, files, path, training_data_split = 0.05, input_width = 25, ensemble_comparison = False):
        ...
        self.data_dict, self.training_names, training_sizes, comparison_sizes = PreloadData(files, path, ensemble_comparison, input_width)
        ...
        for training_name in self.training_names:
    ...
    def __getitem__(self, indexes, transform=False):
        ...
        paramset = self.training_names[paramset_ind]
